...
def get_number_from_img(im, draw=False):
    ...
    # cv2.waitKey(0)
    _image, contours,hierarchy = cv2.findContours(thresh,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
    ...
    if biggest != None:
        ...
        retval, results, neigh_resp, dists = model.findNearest(roismall, k = 1) #opencv 3 #model.find_nearest(roismall, k = 1)  #opencv 2
